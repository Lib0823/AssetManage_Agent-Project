...
- KIS 연동은 mock(VTTC*)/real(TTTC*) TR_ID 변환이 `convertTrId`에 있다는 것과, `account_mode`(MOCK/REAL)가 도메인·TR 라우팅 기준이라는 것을 항상 확인한다. 해외(V로 시작하는 TR: VTTS/VTTT)는 국내와 별도 변환 규칙을 쓴다.
...
- ai-agent가 `POST /api/trading/execute`로 매매를 요청하는 내부 API 계약을 변경할 때는 반드시 ai-pipeline-engineer와 조율한다 — 이 경계면이 깨지면 파이프라인 Stage 6이 조용히 실패한다.
...
## 에러 핸들링
- 테스트 실패 시 원인을 밝히지 않고 스킵하지 않는다. `_docs/STATUS.md`에 "예외 체계 변경 후 일부 테스트가 stale"이라는 알려진 이슈가 있으니, 실패가 이 때문인지 새 회귀인지 구분해서 보고한다.
