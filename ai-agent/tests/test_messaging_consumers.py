...
class TestPipelineRunConsumer:
    ...
    async def test_runs_complete_pipeline_with_trade_date(self, consumer, orchestrator):
        await consumer.handle({'tradeDate': '2026-08-09', 'triggerType': 'MANUAL'})

        orchestrator.run_complete_pipeline.assert_awaited_once_with(trade_date=date(2026, 8, 9))

    async def test_last_run_is_recorded(self, consumer):
        await consumer.handle({'tradeDate': '2026-08-09', 'triggerType': 'SCHEDULED'})

        assert consumer.last_run['trade_date'] == '2026-08-09'
    ...
    async def test_messages_are_processed_one_at_a_time(self, consumer, orchestrator):
        ...
        await consumer._process(_message({'tradeDate': '2026-08-09', 'triggerType': 'SCHEDULED'}))
        await consumer._process(_message({'tradeDate': '2026-08-09', 'triggerType': 'MANUAL'}, offset=1))
